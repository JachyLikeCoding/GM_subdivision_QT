...
void SubdivisionProcessor::subdivision(int level, QQueue<Vertex *> &vqueue, QQueue<Edge *> &equeue, QQueue<Face *> &fqueue) {
...
void SubdivisionProcessor::subdivision4(int level, QQueue<Vertex *> &vqueue, QQueue<Edge *> &equeue, QQueue<Face *> &fqueue) {
	Face *currentface = fqueue.head();
	//just for debug:
	...
	while (currentface->level < level) {
		...
		fqueue.dequeue();
		currentface = fqueue.head();

		if (currentface->level > currentlevel)
		{
			while (vqueue.head()->level == currentlevel) {
				vqueue.dequeue();
			}
			while (equeue.head()->level == currentlevel) {
				equeue.dequeue();
			}
			currentlevel = currentface->level;
		}
	}
	qDebug() << "--------------------subdivision finished!!-------------------" << endl;
}


void SubdivisionProcessor::subdivision3(int level, QQueue<Vertex *> &vqueue, QQueue<Edge *> &equeue, QQueue<Face *> &fqueue) {
	Face *currentface = fqueue.head();
	int currentlevel = currentface->level;
	int nextlevel;
	while (currentface->level < level) {
